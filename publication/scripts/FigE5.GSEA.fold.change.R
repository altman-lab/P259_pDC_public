...
FC.plot.dat <- h.combo.plot.lab %>% 
  ...
  mutate(group1 = recode_factor(factor(group),
         ...
         group2 = recode_factor(factor(group),
            ...
            "P259.1_treat.virus"='"EOS sup vs none"',
            "P259.2_treat.virus"='"Anti-IL-5/5R"*alpha~"vs none"',
            "P259.1_treat.media"='"EOS sup vs none"',
            "P259.2_treat.media"='"Anti-IL-5/5R"*alpha~"vs none"')) %>% 
  #Reorder terms
  mutate(term = gsub("HALLMARK_", "", term)) %>% 
  #Reorder terms
...
fc.plot.all <- plot_grid(plot.v, plot.t, ncol=2, rel_widths = c(0.6,1),
                         labels=c("A","B"))
#fc.plot.all
ggsave("publication/fig/FigE5.GSEA.fold.change.pdf", fc.plot.all, 
       width = 20, height = 15)