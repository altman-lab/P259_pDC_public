gsea.plot <- function(gsea, fdr.cut = 0.1, prefix,
                      print.plot = TRUE, save.plot = FALSE, 
                      height = 5, width = 11, outdir=NULL){
  require(tidyverse)
  require(cowplot)
  ...
  h.plot.dat <- gsea %>% 
    ...
    mutate(group1 = recode_factor(factor(group),
                                  "AntiIL5_HRV.AntiIL5_none.2"="+ Anti-IL5 therapy",
                                  "none_HRV.none_none.2"="- Anti-IL5 therapy",
                                  "EOS.supp_HRV.EOS.supp_none.1"="+ EOS supernatant",
                                  "none_HRV.none_none.1"="- EOS supernatant",
           ...
           group2 = recode_factor(factor(group),
                                  "none_HRV.none_none.1"="RV-infected vs media",
                                  "none_HRV.none_none.2"="RV-infected vs media",
                                  "EOS.supp_HRV.EOS.supp_none.1"="RV-infected vs media",
                                  "AntiIL5_HRV.AntiIL5_none.2"="RV-infected vs media",
                                  
                                  "EOS.supp_HRV.none_HRV.1"="EOS supernatant vs none",
                                  "AntiIL5_HRV.none_HRV.2"="Anti-IL5 therapy vs none",
                                  "EOS.supp_none.none_none.1"="EOS supernatant vs none",
                                  "AntiIL5_none.none_none.2"="Anti-IL5 therapy vs none")) %>% 
  ...
  #Reorder hallmark terms
  if(prefix == "H"){
    h.plot.dat <- h.plot.dat %>% 
      #Reorder terms
  ...
  h.plot1 <- h.plot.dat %>% 
    ...
    facet_grid(pathway~group2, scales="free_y") +
    theme_bw() +
  ...
  if(save.plot){
    ...
    filename <- paste(outdir, prefix, "_GSEA.pdf", sep="")
    ggsave(filename, h.plot.all, width = width, height = height)
  }
  
  if(print.plot){
    plotname <- paste(prefix, "GSEA.plot", sep="_")
    print(h.plot.all)
    assign(plotname, h.plot.all, envir = .GlobalEnv)
  }
}